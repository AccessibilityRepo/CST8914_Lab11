...
accordionBtns.forEach((accordion) => {
  accordion.setAttribute("role", "button"); // Fix 1: Ensure the element behaves like a button for screen readers
  accordion.setAttribute("aria-expanded", "false"); // Fix 2: Default state of the accordion is collapsed
  accordion.setAttribute("tabindex", "0"); // Fix 3: Make the accordion keyboard-focusable

  // Link the button to the content it controls
  let content = accordion.nextElementSibling;
  const uniqueId = content.id || "content-" + Math.random().toString(36).substr(2, 9);
  content.id = uniqueId; // Fix 4: Ensure content has a unique ID
  accordion.setAttribute("aria-controls", uniqueId); // Fix 5: Link button to the content it controls
  ...
  accordion.onclick = function () {
    ...
    if (content.style.maxHeight) {
      ...
      this.setAttribute("aria-expanded", "false"); // Fix 6: Update ARIA state to reflect closed state
    } else {
      //if the accordion is currently closed
      content.style.maxHeight = content.scrollHeight + "px";
      this.setAttribute("aria-expanded", "true"); // Fix 7: Update ARIA state to reflect open state
  ...
  accordion.onkeydown = function (event) {
    if (event.key === "Enter" || event.key === " ") {
      // Fix 8: Handle keyboard interaction for Enter and Space keys
      event.preventDefault(); // Fix 9: Prevent default action (e.g., scrolling with Space)
      this.click(); // Fix 10: Trigger the click event programmatically to toggle the accordion
    }
  };
});
